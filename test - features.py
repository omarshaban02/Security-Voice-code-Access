...
def record_voice(duration=5, sr=22050):
...
def extract_features(audio_data, sr=22050, n_fft=2048, hop_length=512):
    ...
    df_spectral_contrast = pd.DataFrame(data=spectral_contrast.T, columns=[f'spectral_contrast_{i}' for i in range(spectral_contrast.shape[0])])
